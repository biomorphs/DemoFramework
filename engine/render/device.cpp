...
namespace Render
{
		...
		: m_window( theWindow )
	{
		...
		RENDER_ASSERT(glewError == GLEW_OK, "Failed to initialise glew: %s", glewGetErrorString(glewError));
		...
		RENDER_PROCESS_GL_ERRORS("Device Initialise");
	...
	void Device::SetBlending(bool enabled)
	{
		if (enabled)
		{
			// Todo - separate
			glEnable(GL_BLEND);
			RENDER_PROCESS_GL_ERRORS("glEnable");
			glBlendEquation(GL_FUNC_ADD);
			RENDER_PROCESS_GL_ERRORS("glBlendEquation");
			glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
			RENDER_PROCESS_GL_ERRORS("glBlendFunc");
		}
		else
		{
			glDisable(GL_BLEND);
			RENDER_PROCESS_GL_ERRORS("glDisable");
		}
	}
	...
	void Device::SetBackfaceCulling(bool enabled, bool frontFaceCCW)
	{
		...
		RENDER_PROCESS_GL_ERRORS("glFrontFace");
	}
	...
	void Device::SetDepthState(bool enabled, bool writeEnabled)
	{
		...
		RENDER_PROCESS_GL_ERRORS("glDepthMask");
	}

	void Device::ClearColourDepthTarget(const glm::vec4& colour)
	{
		glClearColor(colour.r, colour.g, colour.b, colour.a);
		RENDER_PROCESS_GL_ERRORS("glClearColor");
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		RENDER_PROCESS_GL_ERRORS("glClear");
	}
	...
	void Device::SetArraySampler(uint32_t uniformHandle, uint32_t textureHandle, uint32_t textureUnit)
	{
		...
		RENDER_ASSERT(uniformHandle != -1);
		RENDER_ASSERT(textureHandle != 0);

		glActiveTexture(GL_TEXTURE0 + textureUnit);
		RENDER_PROCESS_GL_ERRORS("glActiveTexture");

		glBindTexture(GL_TEXTURE_2D_ARRAY, textureHandle);
		RENDER_PROCESS_GL_ERRORS("glBindTexture");

		glUniform1i(uniformHandle, textureUnit);
		RENDER_PROCESS_GL_ERRORS("glUniform1i");
	}
	...
	void Device::SetUniformValue(uint32_t uniformHandle, const glm::mat4& matrix)
	{
		...
		RENDER_PROCESS_GL_ERRORS("glUniformMatrix4fv");
	}
	...
	void Device::DrawPrimitives(PrimitiveType primitive, uint32_t vertexStart, uint32_t vertexCount)
	{
		...
		RENDER_ASSERT(primitiveType != -1);

		glDrawArrays(primitiveType, vertexStart, vertexCount);
		RENDER_PROCESS_GL_ERRORS("glDrawArrays");
	}
}