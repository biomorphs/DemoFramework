...
namespace Render
{
	...
	{
		RENDER_ASSERT(srcBuffer != nullptr);
		RENDER_ASSERT(components <= 4);
	...
	bool VertexArray::Create()
	{
		...
		RENDER_PROCESS_GL_ERRORS_RET("glGenVertexArrays");

		glBindVertexArray(m_handle);
		RENDER_PROCESS_GL_ERRORS_RET("glBindVertexArray");
		...
		for (auto it = m_descriptors.begin(); it != m_descriptors.end(); ++it)
		{
			...
			RENDER_PROCESS_GL_ERRORS_RET("glVertexAttribPointer");

			// Enable the stream
			glEnableVertexAttribArray(it->m_attribIndex);
			RENDER_PROCESS_GL_ERRORS_RET("glEnableVertexAttribArray");
		}
	...
	void VertexArray::Destroy()
	{
		...
		if (m_handle != 0)
		{
			...
			RENDER_PROCESS_GL_ERRORS("glDeleteVertexArrays");
		}		
